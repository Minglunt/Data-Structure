...
public class ComplexOomage implements Oomage {
    ...
    public int hashCode() {
        int total = 0;
        for (int x : params) {
/*          We should not use 256 as the base because it will cause
            the overflow to be 0 (see hint for more details) */
//          total = total * 256;
//          So we should pick up another base number such as 31
            total = total * 31;
            total = total + x;
        }
        return total;
    ...
    public static ComplexOomage randomComplexOomageForOverflow() {
